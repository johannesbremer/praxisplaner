 ...
 *
 * ============================================================
 * These tests document the EXPECTED behavior of the rules engine, even though
 * the implementation is not yet complete. Each test includes a German description
 * (generated by generateRuleName) that documents what the rule should do.
 *
 * - Some tests currently PASS (basic functionality already implemented)
 * - Some tests currently FAIL (features not yet implemented)
 * - All tests document the expected behavior for implementation
 *
 * Test Structure:
...
describe("Rule Engine: Numeric Comparison Conditions", () => {
  ...
  test("CONCURRENT_COUNT with practice scope - should block when too many concurrent appointments", async () => {
    ...
    // Create rule: Block if >= 2 concurrent "Checkup" appointments at practice level
    // Note: This is simplified - in the real implementation, CONCURRENT_COUNT would need
    // appointmentTypes array and scope field in the condition tree
    const conditionTree = {
...
describe("E2E: Slot Generation with Rules", () => {
  ...
  test("Complex nested: Block Surgery AND >14 days ahead AND 2+ concurrent appointments", async () => {
    ...
    // Create schedule for Mondays and Thursdays
    await createBaseSchedule(
      t,
      practiceId,
      ruleSetId,
    ...
    // Create rule: Block if Surgery AND >= 14 days ahead AND >= 2 concurrent
    const conditionTree = {
      conditions: [
        {
          conditionType: "APPOINTMENT_TYPE",
    ...
    // Test: Surgery 20 days ahead with 2+ concurrent should be blocked
    // Note: This will likely fail because concurrent count checking isn't implemented yet
    const surgerySlots = await t.query(api.scheduling.getSlotsForDay, {
    ...
    // Expected to be blocked, but may not be implemented yet
    expect(surgerySlots.slots.length).toBeGreaterThan(0);
    // This assertion will likely fail - that's expected for TDD
    expect(surgerySlots.slots.every((slot) => slot.status === "BLOCKED")).toBe(
      true,
    );
  });
