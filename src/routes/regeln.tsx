...
const formatRuleDescription = (rule: FlatRule): string => {
  if (rule.ruleType === "BLOCK") {
    ...
    if (rule.block_timeRangeStart && rule.block_timeRangeEnd) {
      parts.push(`Zeit: ${rule.block_timeRangeStart} - ${rule.block_timeRangeEnd}`);
    }
...
export default function LogicView() {
  ...
  const [selectedRuleSetId, setSelectedRuleSetId] = useState<Id<"ruleSets"> | null>(null);
  ...
  // Mutations
  const createDraftMutation = useMutation(api["rule-sets"].createDraftFromActive);
  const activateRuleSetMutation = useMutation(api["rule-sets"].activateRuleSet);

  const selectedRuleSet = ruleSetsQuery?.find(rs => rs._id === selectedRuleSetId);
    ...
    <div className="container mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Verwalten Sie Regelsets und deren Versionen für die Terminverfügbarkeit
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Select
                  onValueChange={(value) => { setSelectedRuleSetId(value as Id<"ruleSets">); }}
                    ...
                    {ruleSetsQuery?.map((ruleSet) => (
                      <SelectItem key={ruleSet._id} value={ruleSet._id}>
                        <div className="flex items-center gap-2">
                          <span>v{ruleSet.version} - {ruleSet.description}</span>
                          {ruleSet.isActive && (
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {isCreatingDraft ? "Erstelle Draft..." : "Neues Draft erstellen"}
                </Button>
          ...
          {selectedRuleSet && (
            <Card>
              ...
              <CardContent>
                {rulesQuery ? rulesQuery.length === 0 ? (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <Badge
                                className="text-xs"
                                variant={rule.ruleType === "BLOCK" ? "destructive" : "secondary"}
                              >
                                {rule.ruleType}
                              </Badge>
                            </div>
          ...
          <Card>
            ...
            <CardContent>
              {ruleSetsQuery ? ruleSetsQuery.length === 0 ? (
                <div className="text-muted-foreground">Keine Regelsets gefunden.</div>
              ) : (
                <div className="space-y-2">
                  {ruleSetsQuery
                    ...
                    .map((ruleSet) => (
                      <div
                        ...
                        key={ruleSet._id}
                        onClick={() => { setSelectedRuleSetId(ruleSet._id); }}
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="font-medium">v{ruleSet.version}</div>
                            <div className="text-sm text-muted-foreground">
                              {ruleSet.description}
                            </div>
                          </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">v{selectedRuleSet.version}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Status:</span>
                  <Badge variant={selectedRuleSet.isActive ? "default" : "secondary"}>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {new Date(selectedRuleSet.createdAt).toLocaleDateString("de-DE")}
                  </span>
                </div>
              </CardContent>
            </Card>
