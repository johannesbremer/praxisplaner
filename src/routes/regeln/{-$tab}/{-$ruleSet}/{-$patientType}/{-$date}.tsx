...
export default function LogicView() {
  ...
  const [selectedRuleSetId, setSelectedRuleSetId] =
    useState<Id<"ruleSets"> | null>(urlSelectedRuleSetId);
  ...
  const [simulatedContext, setSimulatedContext] = useState<SimulatedContext>({
    ...
    patient: { isNew: urlPatientIsNew },
  });
  const [selectedDate, setSelectedDate] = useState<Date>(urlSelectedDate);
  ...
  React.useEffect(() => {
    if (currentWorkingRuleSet) {
      ...
      // Update URL if the rule set changed
      if (currentWorkingRuleSet._id !== urlSelectedRuleSetId) {
        updateUrl({ ruleSet: currentWorkingRuleSet._id });
      }
    } else if (urlSelectedRuleSetId) {
      setSimulationRuleSetId(undefined);
      updateUrl({ ruleSet: null });
    }
  }, [currentWorkingRuleSet, urlSelectedRuleSetId, updateUrl]);
  ...
  const handleVersionClick = React.useCallback(
    ...
    [currentPractice, unsavedRuleSet, selectedRuleSetId, updateUrl],
  );
  ...
  const handleActivateRuleSet = async (
    ...
    try {
      ...
      if (pendingRuleSetId) {
        ...
        updateUrl({ ruleSet: pendingRuleSetId });
      } else {
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-6">
              ...
              <SimulationControls
                ...
                onSimulatedContextChange={(newContext) => {
                  setSimulatedContext(newContext);
                  updateUrl({ patientType: newContext.patient.isNew });
                }}
                onSimulationRuleSetChange={(newRuleSetId) => {
                  setSimulationRuleSetId(newRuleSetId);
                  updateUrl({ ruleSet: newRuleSetId });
                }}
