...
export default function LogicView() {
  ...
  const activeTabFromUrl = mapParamToTab(params.tab);
  const simulationRuleSetIdFromUrl = params.ruleSet as
    | Id<"ruleSets">
    | undefined;
  const patientTypeFromUrl = mapParamToPatientType(params.patientType);
  const selectedDateFromUrl = parseDateParam(params.date);
  ...
  // Simulation state - now driven by URL parameters
  const [activeTab, setActiveTabInternal] = useState<string>(activeTabFromUrl);
  const [simulatedContext, setSimulatedContextInternal] =
    useState<SimulatedContext>({
      appointmentType: "Erstberatung",
      patient: { isNew: patientTypeFromUrl },
    });
  const [selectedDate, setSelectedDateInternal] =
    useState<Date>(selectedDateFromUrl);
  const [selectedSlot, setSelectedSlot] = useState<null | SlotDetails>(null);
  const [simulationRuleSetId, setSimulationRuleSetIdInternal] = useState<
    Id<"ruleSets"> | undefined
  >(simulationRuleSetIdFromUrl);

  // Update state when URL changes
  useEffect(() => {
    setActiveTabInternal(activeTabFromUrl);
  }, [activeTabFromUrl]);

  useEffect(() => {
    setSimulatedContextInternal((prev) => ({
      ...prev,
      patient: { isNew: patientTypeFromUrl },
    }));
  }, [patientTypeFromUrl]);
  ...
  useEffect(() => {
    setSelectedDateInternal(selectedDateFromUrl);
  }, [selectedDateFromUrl]);

  useEffect(() => {
    setSimulationRuleSetIdInternal(simulationRuleSetIdFromUrl);
  }, [simulationRuleSetIdFromUrl]);
  ...
  const updateUrl = useCallback(
    (updates: {
      date?: Date;
      patientType?: boolean;
      ruleSet?: Id<"ruleSets"> | undefined;
      tab?: string;
    }) => {
      const tabToUse = updates.tab ?? activeTab;
      const ruleSetToUse = updates.ruleSet ?? simulationRuleSetId;
      const patientTypeToUse =
        updates.patientType ?? simulatedContext.patient.isNew;
      const dateToUse = updates.date ?? selectedDate;

      const tabParam = mapTabToParam(tabToUse);
      const ruleSetParam = ruleSetToUse || undefined; // undefined means active rule set
      const patientTypeParam = mapPatientTypeToParam(patientTypeToUse);
      const dateParam = isToday(dateToUse)
        ? undefined
        : formatDateParam(dateToUse);

      // Build params object without undefined values to satisfy TypeScript
      const params: {
        date?: string;
        patientType?: string;
        ruleSet?: string;
      ...
      if (ruleSetParam) {
        params.ruleSet = ruleSetParam;
      }
      if (patientTypeParam) {
        params.patientType = patientTypeParam;
      }
      if (dateParam) {
      ...
      void navigate({
        params,
        replace: true, // Use replace to avoid creating excessive history entries
  ...
  // Wrapper functions that also update URL
  ...
  const setSimulatedContext = useCallback(
    (context: SimulatedContext) => {
      setSimulatedContextInternal(context);
      updateUrl({ patientType: context.patient.isNew });
    },
    [updateUrl],
  );
  ...
  const setSimulationRuleSetId = useCallback(
    (ruleSetId: Id<"ruleSets"> | undefined) => {
      setSimulationRuleSetIdInternal(ruleSetId);
      updateUrl({ ruleSet: ruleSetId });
    },
    [updateUrl],
  );
  ...
  }, [currentWorkingRuleSet, setSimulationRuleSetId]);
